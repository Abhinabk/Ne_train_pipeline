...
def stations_needing_update(con) -> pd.DataFrame:
    ...
    stations_info["latest_weather_date"] = pd.to_datetime(
        stations_info["latest_weather_date"]
    )
    stations_info["latest_train_date"] = pd.to_datetime(
        stations_info["latest_train_date"]
    )
...
def update_weather(con):
    ...
    for row in df.itertuples():
        ...
        print(f"[LOG] Running Incremental {station_code} {start_date} -> {end_date}")
        try:
            data = fetch_weather_daily(
                station_code, latitude, longitude, start_date, end_date
            )
